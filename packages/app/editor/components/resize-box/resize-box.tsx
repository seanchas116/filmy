...
import { usePointerStroke } from "../use-pointer-stroke";
  ...
  return (
    <>
      <rect
        ...
        height={rect.height}
        fill="none"
        stroke={props.stroke ?? colors.accent}
        pointerEvents="none"
      />
...
const ResizeEdgeHandle = (props: ResizeEdgeHandleProps) => {
  ...
  const resize = useCallback((mousePos: Vec2, proportional?: boolean) => {
    const offset = mousePos.sub(state.dragStartMousePos);

    // TODO: snapping

    const points = state.dragStartPoints.map((p) => new Vec2(p.x, p.y));
    const startWidth = Math.abs(points[0].x - points[1].x);
    const startHeight = Math.abs(points[0].y - points[1].y);

    points[props.placement][props.axis] += offset[props.axis];

    if (proportional) {
      const crossAxis = props.axis === "x" ? "y" : "x";
      const ratio =
        props.axis === "x"
          ? startHeight / startWidth
          : startWidth / startHeight;
      const delta = (offset[props.axis] * ratio) / 2;
      points[props.placement][crossAxis] += delta;
      points[1 - props.placement][crossAxis] -= delta;
    }
  ...
  const handleKeyDown = useCallback(
    (e: KeyboardEvent) => {
      if (lastMousePos.current && e.key === "Shift") {
        resize(lastMousePos.current, true);
      }
    },
    [resize]
  );

  const handlleKeyUp = useCallback((e: KeyboardEvent) => {
    if (lastMousePos.current && e.key === "Shift") {
      resize(lastMousePos.current, false);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      window.removeEventListener("keyup", handlleKeyUp);
    };
  }, []);
...
const ResizeCornerHandle = (props: ResizeCornerHandleProps) => {
  ...
  const resize = useCallback((mousePos: Vec2, proportional?: boolean) => {
    ...
    if (proportional) {
      const { x, y } = proportionalResize(p0, p1, xs, ys, placement);
      xs[placement.x] = x;
      ys[placement.y] = y;
    }
